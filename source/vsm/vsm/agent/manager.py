...
class AgentManager(manager.Manager):
    ...
    def update_keyring_admin_from_db(self, context):
        ...
        if keyring_admin:
			...
			# add by yanghc 20170329
	        # for add the file ceph.client.admin.keyring
            utils.write_file_as_root(FLAGS.client_keyring_admin, keyring_admin, "w")
            return True
        else:
    ...
    def write_keyring(self, context):
        """write bootstrap-type keyrings."""
        return self.ceph_driver.write_keyring()

    def build_ceph_dir(self, context):
        """Build ceph dir."""
        return self.ceph_driver.build_ceph_dir()
    ...
    def osd_remove(self, context, osd_id):
        ...
        # add by yanghc 20170406
        # Remove it from db
        db.osd_delete(context, osd_id)
        db.device_delete(context, osd['device']['id'])       
        return True
    ...
    def get_journal_part_dev(self,dev_used_by_ceph,name_by_path_dict,name_by_uuid_dict,name_by_partuuid_dict):
        dev_journal = []
        for disk in dev_used_by_ceph:
            disk,err = utils.execute("ls",disk,"-l",run_as_root=True)
            disk = disk.split("\n")[0]
            disk = disk.split(" ")[-1]
            if disk.find('/by-path/') != -1 :
                path_index = name_by_path_dict.values().index(disk)
                dev = name_by_path_dict.keys()[path_index]
                dev_journal.append(dev)
            elif disk.find('/by-uuid/') != -1 :
                uuid_index = name_by_uuid_dict.values().index(disk)
                dev = name_by_uuid_dict.keys()[uuid_index]
                dev_journal.append(dev)
            elif disk.find('/by-partuuid/') != -1:
                partuuid_index = name_by_partuuid_dict.values().index(disk)
                dev = name_by_partuuid_dict.keys()[partuuid_index]
                dev_journal.append(dev)
            else:
                dev_journal.append(disk)
        return dev_journal
    
    def get_available_physical_disks(self, context):
        '''
        2018-3-29 change another way to get available_disk_name
        2018-4-2 add the search of /dev/disk/by-partuuid/
        2018-4-25 modify the fuctions of "get availible disks"(add osds)
        '''
        #LOG.info('333333333')
        available_disk_name = self.ceph_driver.get_available_physical_disks(context)
        LOG.info('available_disk_name=====%s'%available_disk_name)
        devices = db.device_get_all_by_service_id(context,self._service_id)
        dev_used_by_ceph = [dev.journal for dev in devices]
        available_disk_info_list = []
        name_by_path_dict = self.ceph_driver.get_disks_name_by_path_dict(available_disk_name)
        name_by_uuid_dict = self.ceph_driver.get_disks_name_by_uuid_dict(available_disk_name)
        name_by_partuuid_dict = self.ceph_driver.get_disks_name_by_partuuid_dict(available_disk_name)

        # remove disk which is used by ceph journal
        parted_dev_used_by_ceph = self.get_journal_part_dev(dev_used_by_ceph,name_by_path_dict,name_by_uuid_dict,name_by_partuuid_dict)
        LOG.info("the disk used by ceph=====%s"%parted_dev_used_by_ceph)
        dev_replace = []
        for disk in available_disk_name:
            disk = disk.split("../")[-1]
            for parted_dev in parted_dev_used_by_ceph:
#                if parted_dev.find(disk) == 0:
#                   available_disk_name.remove(disk)
#                   break
                if parted_dev.find(disk) != -1:
                    dev_replace.append(disk)
        for dev_replace_cell in dev_replace:
            available_disk_name = available_disk_name.remove(dev_replace_cell)
    
        for disk in available_disk_name:
            disk_dev,err = utils.execute("ls",disk,"-l",run_as_root=True)
            disk_dev = disk_dev.split("\n")[0]
            disk_dev = "/dev/" + disk_dev.split(" ../")[-1]
            LOG.info("disk name=====%s"%disk_dev)
            by_path_name = name_by_path_dict.get(disk_dev,'')
            by_uuid_name = name_by_uuid_dict.get(disk_dev,'')
            by_partuuid_name = name_by_partuuid_dict.get(disk_dev,'')
            available_disk_info_list.append({'disk_name':disk,
                                             'by_path':by_path_name,
                                             'by_uuid':by_uuid_name,
                                             'by-partuuid':by_partuuid_name, })
        LOG.info('available_disk_info_list=====%s'%available_disk_info_list)
        return available_disk_info_list
    ...
    def add_new_physical_disks_to_cluster(self, context, body):
        '''
        2018-4-2 rewrite add_new_physical_disks_to_cluster
        2018-5-10 add new parameter dev_id to update disk info
        '''
#        all_disk = glob.glob('/dev/disk/by-path/*')
#        for disk in body['osdinfo']:
#            disk['data'] = disk['physical_device']+'1'
#            disk['journal'] = disk['physical_device']+'2'
#            osd_id = self.add_disk_to_db(context,disk)
#            physical_disk = disk['physical_device']
#            self.osd_add_by_physical_disks(context,physical_disk,osd_id)
        host_name = body['server_name']
        value = {}
        for disk in body['osdinfo']:
            disk['data'] = disk['physical_device']+'1'
            disk['journal'] = ''
            osd_id,dev_id = self.add_disk_to_db(context,disk)
            physical_disk = disk['physical_device']
            osd_id = self.osd_add_by_physical_disks(context,physical_disk,osd_id)
            value['journal'] = '/var/lib/ceph/osd/ceph-'+str(osd_id)+'/journal'
            db.device_update(context, dev_id, value)
        return True

    def osd_add_by_physical_disks(self, context, physical_disk, osd_id):
        return self.ceph_driver.add_osd_by_physical_disk(context, host_id=None,
                            physical_disk=physical_disk, osd_id_in=osd_id)
    ...
    def _modify_init_nodes_from_config_to_db(self,context,osd_info,mon_info,crushmap):
        ...
        for server in servers:
            ...
            #modify by yanghuich 20150405 
            # for if zone_id is none, when add osd is error
            if server['zone_id'] is None :
                server['zone_id'] = 1
...
#                node = socket.gethostbyaddr(cluster_ip)[0]
                node_hosts,err = utils.execute("cat","/root/hosts",run_as_root=True)
                LOG.info("node_hosts = %s"%node_hosts)
                node_hosts = str(node_hosts).split('\n')
                LOG.info("node_hosts = %s"%node_hosts)
                for host in node_hosts:
                    if host.find(cluster_ip) != -1:
                        node = host.split(' ')[-1]
                        break
                addr_cache[cluster_ip]['host'] = unicode(node)
                LOG.info("node = %s"%node)
                LOG.info("addr_cache = %s"%addr_cache)
    ...
    def check_pre_existing_ceph_conf(self, context, body):
        ...
        # else:
        #     pass
        #  modify by yanghc 2017-03-01
        # for the latest ceph version(jewel), the fields needs the underline
