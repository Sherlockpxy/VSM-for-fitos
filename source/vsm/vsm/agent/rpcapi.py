...
class AgentAPI(vsm.openstack.common.rpc.proxy.RpcProxy):
    ...
    def write_keyring(self, context, host):
        topic = rpc.queue_get_for(context, self.topic, host)
        res = self.call(context,
                        self.make_msg('write_keyring'),
                        topic,
                        version='1.0', timeout=6000)
        return res

    def build_ceph_dir(self, context, host):
        topic = rpc.queue_get_for(context, self.topic, host)
        res = self.call(context,
                        self.make_msg('build_ceph_dir'),
                        topic,
                        version='1.0', timeout=6000)
        return res
    ...
    def get_available_physical_disks(self, context, host):
        topic = rpc.queue_get_for(context, self.topic, host)
        res = self.call(context,
                        self.make_msg('get_available_physical_disks'),
                        topic, version='1.0', timeout=6000)
        return res
    ...
    def add_new_physical_disks_to_cluster(self, context, body, host):
        topic = rpc.queue_get_for(context, self.topic, host)
        res = self.call(context,
                        self.make_msg('add_new_physical_disks_to_cluster',
                                      body=body),
                        topic,
                        version='1.0', timeout=6000)
