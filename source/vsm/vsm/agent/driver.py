...
#            self.update_etc_fstab(context)
    ...
    def get_ceph_disk_list(self):
        '''
        Execute 'sudo cat /proc/mounts | grep ceph' and gather ceph partition info.
        :return: a python data structure containing the content of the command above
        '''
...
#          else self.v08_ceph_disk_list_parser(output)
        output = utils.execute('df', '-h',  run_as_root=True)[0]
        return self.ceph_mounts_parser(output)
    ...
    def ceph_mounts_parser(self, output):
        '''
        Parse the output of 'df -h'.
        :param output: the output to be parsed.
        :return: a list of disk-info dictionaries.
        '''
        disk_list = []
    ...
    def add_osd_by_physical_disk(self, context, host_id, physical_disk, osd_id_in=None):
        ''' 
        2018-3-28 support flashcache
        2018-5-7 add new way to get host_obj when osd_ini_in=None
        '''
        flashcache_flag = 0
        flashcache_mod,err = utils.execute('lsmod',run_as_root=True)
        flashcache_mod = flashcache_mod.split('\n')
        for flashcache_mod_cell in flashcache_mod:
            if flashcache_mod_cell.find("flashcache") != -1:
                flashcache_flag = 1
                break
        ...
        for strg in strg_list:
            ...
            # other steps by yanghc
            # step 2
            LOG.info('>>> step2 start:add osd to cluster')
            cluster_name = db.cluster_get(context, strg['cluster_id']).name        
            file_system = 'xfs'
            arglist = ['ceph-disk', 'zap', physical_disk]
            output = utils.execute(*arglist,run_as_root=True)[0]
            if strg['file_system']:
                file_system = strg['file_system']
            if flashcache_flag == 0:
                arglist = ['ceph-disk', 'prepare', '--cluster',cluster_name, '--fs-type',
                           file_system, '--', physical_disk]
            else:
                arglist = ['ceph-disk', 'myprepare', '--cluster',cluster_name, '--fs-type',
                           file_system, '--', physical_disk]
            output = utils.execute(*arglist,run_as_root=True)[0]
            arglist = ['ceph-disk', 'activate', physical_disk+'1']
            output = utils.execute(*arglist,run_as_root=True)[0]            
       
            time.sleep(3) 
            osd_id = self.get_physical_disk_osd_id(physical_disk)             
            
            LOG.info('   add osd osd_id success: %s' % osd_id)
            ...
            # other steps
            LOG.info('>> _add_osd start ')
            self._add_osd_to_crushmap(context,
                          osd_id,
                          crush_dict,
                          osd_conf_dict,
                          osd_state)
    ...
    def _add_osd_to_crushmap(self,context,osd_id,crush_dict,osd_conf_dict,osd_state):
        # step 3.1
        ...
        osd_weight = osd_state.get('weight')
        if osd_weight:
             weight = osd_weight
        LOG.info("osd_weight=======%s" %osd_weight )
        utils.execute("ceph", "osd", "crush", "add", "osd.%s" % osd_id, weight,
                 osd_location_str,
                 run_as_root=True)
        return True
        
    def get_physical_disk_osd_id(self, physical_disk):
        ...
        for mounted_disk in mounted_disk_info:
            ...
            if mounted_disk_list[0].find(physical_disk) != -1 and  mounted_disk_list[2].find('ceph') != -1:
                osd_path = mounted_disk_list[2]
                break
        osd_id = self.get_osd_whoami(osd_path)
        # stdout = utils.execute('cat','%s/whoami' %osd_path,run_as_root=True)[0]
        # osd_id = str(int(stdout))
        return osd_id
    ...
    def _add_osd(self,
        ...
        try:
            osd_data_path = self.get_ceph_config(context)['osd']['osd_data']
            if "$cluster" in osd_data_path :
                cluster_name = db.cluster_get(context,osd_state['cluster_id']).name
                osd_pth = osd_data_path.replace('$id',osd_id).\
                               replace('$cluster',cluster_name)
            else:
                osd_pth = osd_data_path.replace('$id',osd_id)
	    LOG.info('>>>>>>yhc test:osd add osd_pth =%s'%osd_pth)
        except:
            osd_pth = os.path.join(FLAGS.osd_data_path, "ceph-" + osd_id)
        ...
        try:
            ...
            LOG.info('>>>>>>yhc test:osd add keyring path=%s'%osd_keyring_pth)
        except:
            osd_keyring_pth = '/var/lib/ceph/osd/ceph-%s/keyring' % osd_id            
    ...
    def add_monitor(self, context, host_id, mon_id, port="6789"):
        ...
        try:
            mon_data_path = self.get_ceph_config(context)['mon']['mon_data']
            # modify by yanghc 20170325
            # the cluster should be passed in
            if "$cluster" in mon_data_path :
                cluster_id = ser['cluster_id']
                cluster_name = db.cluster_get(context,ser['cluster_id']).name
            ...
            mon_path = os.path.join(FLAGS.monitor_data_path,"ceph-" + mon_id)
    ...
    def remove_monitor(self, context, host_id, is_stop=False):
        ...
        try:
            ...
            pass
        # step 5
        # add by yanghc 20170328
        LOG.info('>> remove ceph mon step5 start')
        try:
            mon_data_path = self.get_ceph_config(context)['mon']['mon_data']
            if "$cluster" in mon_data_path :
                cluster_name = db.cluster_get(context,cluster_id).name
                mon_path = mon_data_path.replace('$id',mon_id).\
                               replace('$cluster',cluster_name)
            else:
                mon_path = mon_data_path.replace('$id',mon_id)
            #LOG.info('remove mon : mon_pth =%s'%mon_path)
        except:
            mon_path = os.path.join(FLAGS.monitor_data_path,"ceph-" + mon_id)

        mon_data_deleted_path = "/var/lib/ceph/mon-removed"
        utils.execute('ssh', '-t', 'root@'+host,
                      'mkdir', '-p', mon_data_deleted_path,
                              run_as_root=True)
        fn = '{cluster}-{mod_id}-{stamp}'.format(
            mod_id=mon_id,
            cluster=cluster_name,
            stamp=datetime.datetime.utcnow().strftime("%Y-%m-%dZ%H:%M:%S"),
            )
        mon_deleted_path = mon_data_deleted_path+"/"+fn
        utils.execute('ssh', '-t', 'root@'+host,
                      'mv',mon_path,mon_deleted_path,run_as_root=True)
    ...
    def _build_etc_ceph_dirs(self):
        try:
            utils.execute('mkdir', '-p', '/etc/ceph',
                              run_as_root=True)
        except:
            LOG.info('build dirs in /etc/ceph failed!')

    def build_ceph_dir(self):
	"""Build ceph dir:
           /etc/ceph
	   /var/lib/ceph
        """
        self._build_lib_ceph_dirs()
        self._build_etc_ceph_dirs()
...
#        self.update_etc_fstab(context)                              # anything yet to change the state of the system...
    ...
    def get_available_physical_disks(self, context):
        """
        2018-3-28 support flashcache
        """
        flashcache_flag = 0
        ...
        for flashcache_mod_cell in flashcache_mod:
            if flashcache_mod_cell.find("flashcache") != -1:
                ...
                break
        if flashcache_flag == 0:
            LOG.info('flashcache not installed') 
            all_disk_info,err = utils.execute('blockdev','--report',run_as_root=True)
            all_disk_info = all_disk_info.split('\n')
            all_disks = []
            dev_disks= []
            if len(all_disk_info)>1:
                for line in all_disk_info[1:-1]:
                    ...
                    while '' in line_list:
                        line_list.remove('')
                    LOG.info('line_list====%s'%line_list)
                    if int(line_list[-2]) <= 1024:
                        continue
                    if line_list[-1].find('-') != -1:
                        continue
                    if line_list[-3] and int(line_list[-3]) == 0:
                        all_disks.append(line_list[-1])
        else:
            LOG.info('flashcache installed')
            all_disk_info,err = utils.execute('ls','/dev/mapper/','-l',run_as_root=True)
            all_disk_info = all_disk_info.split('\n')
            all_disks = []
            dev_disks= []
            if len(all_disk_info)>1:
                for line in all_disk_info[1:-1]:
                    ...
                    while '' in line_list:
                        line_list.remove('')
                    if line_list[-1].find('-') != -1:
                        if line_list[-3][-1] not in str(range(0,9)):
                            all_disks.append("/dev/mapper/"+line_list[-3])
        LOG.info("all_disks======%s"%all_disks)
        ...
        for mounted_disk in mounted_disk_info:
            mounted_disk_list = mounted_disk.split(' ')
            if mounted_disk_list[0].find('/dev/') != -1:
                for all_disk_cell in all_disks:
                    if mounted_disk_list[0].find(all_disk_cell) != -1:
                        dev_disks.append(all_disk_cell)
                        break
        for dev_disks_cell in dev_disks:
            all_disks.remove(dev_disks_cell)
        LOG.info("unused_disks======%s"%all_disks)
        ...
        for line in pvs_disk_info:
            line_list = line.split(' ')
            if line_list[-1].find('/dev/') != -1:
                for all_disk_cell in all_disks:
                    if line_list[-1].find(all_disk_cell) != -1:
                        dev_disks.append(all_disk_cell)
                        break
        for dev_disks_cell in dev_disks:
            all_disks.remove(dev_disks_cell)
        LOG.info("avaliable disks are %s"%all_disks)
...
#                disk_name_dict['/dev/%s'%(byuuid_list[1])] = '/dev/disk/by-path/%s'%(byuuid_list[0].split(' ')[-1])
                disk_name_dict['/dev/%s'%(byuuid_list[1])] = '/dev/disk/by-uuid/%s'%(byuuid_list[0].split(' ')[-1])
        return disk_name_dict

    def get_disks_name_by_partuuid_dict(self,disk_name_list):
        '''
        2018-4-2 add get_disks_name_by_partuuid_dict
        '''
        disk_name_dict = {}
        by_partuuid_info,err = utils.execute('ls','-al','/dev/disk/by-partuuid',run_as_root=True)
        LOG.info('by_partuuid_info===%s,err===%s'%(by_partuuid_info,err))
        for bypartuuid in by_partuuid_info.split('\n'):
            bypartuuid_list = bypartuuid.split(' -> ../../')
            if len(bypartuuid_list) > 1:
                disk_name_dict['/dev/%s'%(bypartuuid_list[1])] = '/dev/disk/by-partuuid/%s'%(bypartuuid_list[0].split(' ')[-1])
        return disk_name_dict
    ...
    def _remove_osd(self, context, osd_id, host, host_is_running=True):
        ...
        # modify by yanghc 2017-03-22
        # if the os is centos,should start the ceph with systemctl 
        is_systemctl = self._is_systemctl()
        if host_is_running:
	        ...
	        if is_systemctl:
                    utils.execute("systemctl", "stop","ceph-osd@%s" % osd_id,
                              run_as_root=True)
	        else:
