...
class SchedulerManager(manager.Manager):
    ...
    def add_monitor(self, context, server_list):
        ...
        for ser in new_monitor_list:
            try:
                ...
                # modify by yanghc 20170327
                # let the mon_id = host_name
                mon_id = ser['host']
    ...
    @utils.single_lock
    def add_server_fh(self, context, body=None):
        """Add the servers into ceph cluster.
        ...
        _update_ssh_key()
        server_list = body['servers']
        server_id_list=[x.strip() for x in server_list.split(",") if len(x.strip())]
        for ser_id in server_id_list :
            ser_ref = db.init_node_get(context, ser_id)
            mon_id = ser_ref['host']
            LOG.info("new_monitor id %s" % mon_id)

            LOG.info("build ceph dir")
            self._agent_rpcapi.build_ceph_dir(context, ser_ref['host'])
            # Update ceph.conf and keyring.admin from DB.
            LOG.info("add new server save config")
            self._agent_rpcapi.update_ceph_conf(context, ser_ref['host'])
            LOG.info("add new server save keyring")
            self._agent_rpcapi.update_keyring_admin_from_db(context,
                                                            ser_ref['host'])
            self._agent_rpcapi.write_keyring(context,ser_ref['host'])
            values = {'status': 'Active'}
            self._conductor_api.init_node_update(context, ser_id,values)
    ...
    def get_available_disks(self, context, body):
        ...
        if status == 'Active':
            ...
            return res
    def get_available_physical_disks(self, context, body):
        server_id = body['server_id']
        server = db.init_node_get_by_id(context,id=server_id)
        status = server['status']
        if status == 'Active':
            res = self._agent_rpcapi.get_available_physical_disks(context,server['host'])
            return res
    ...
    def add_new_physical_disks_to_cluster(self, context, body):
        ...
        if server_id is not None:
            ...
            server = db.init_node_get_by_host(context,host=server_name)
        self._agent_rpcapi.add_new_physical_disks_to_cluster(context, body, server['host'])
        new_osd_count = int(server["data_drives_number"]) + len(body['osdinfo'])
        values = {"data_drives_number": new_osd_count}
        self._conductor_rpcapi.init_node_update(context,
                                        server["id"],
    ...
    def add_batch_new_physical_disks_to_cluster(self, context, body):
        """
        ...
        try:
            for disk_in_same_server in disks:
                self.add_new_physical_disks_to_cluster(context, disk_in_same_server)
        except:
            return {"message":"data error"}
        return {"message": "success"}
