...
var $ctrlPhysicalDevice = $("#txtPhysicalDevice")[0];
...
//	PostData("get_available_physical_disks",{"server_id":server.node_id});
	PostData("get_available_physical_disks",{"server_id":server.node_id});
...
function SelectDevice(type,device_name){
    ...
    if(type == "journal"){
    	title = "Device Info";
    	$("#txtPhysicalDevice").val(device_name);
    	$("#lblPhysicalDeviceHelp").after(GenerateIcon(type,title,GenerateDeviceInfo(device))); 
    }
...
function AddOSDItemInTable(){
	...
	var osdHtml = "";
		...
		osdHtml += "	<td class='sortable normal_column physical_device'>"+$ctrlPhysicalDevice.value+"</td>";
...
function AddOSD(){
	...
	for(var i=0;i<OSD_Items.length;i++){
		var osd = {
			...
			"physical_device":OSD_Items[i].children[5].innerHTML,
		...
		for(var j=0;j<server_list.length;j++){
			if(osd_list[i].server_name == server_list[j].server_name){
				var osd = {
					...
					"physical_device":osd_list[i].physical_device,
//            		"data":osd_list[i].data
    ...
    PostData("add_new_osd_action_fh",post_data);
}
...
function UpdatePopoverForm(disks){
	...
	$("#selPhysicalDevice")[0].options.length = 0;
//	$("#selDataDevice")[0].options.length = 0;
	for(var i=0;i<DEVICE_LIST.length;i++){
		if(i==0){
			$("#txtPhysicalDevice")[0].value = DEVICE_LIST[i].disk_name;
//			$("#txtDataDevice")[0].value = DEVICE_LIST[i].disk_name;
			//add the device info
//			$(".pop-journal").remove();
			$(".pop-physical").remove();
			$("#lblPhysicalDeviceHelp").after(GenerateIcon("physical","physical Device Info",GenerateDeviceInfo(DEVICE_LIST[i]))); 
...
//		option2.text = DEVICE_LIST[i].disk_name;
		$("#selPhysicalDevice")[0].options.add(option1);
//		$("#selDataDevice")[0].options.add(option2);
	}
}

function ResetForm(){
	$("#txtPhysicalDevice").val("");
	//$("#txtDataDevice").val("");
	$("#txtWeight").val("");
		...
		success: function(data){
			switch(method){
				...
				case "check_device_path":
					...
					break;
				case "check_physical_device_path":
					if(data.status == "OK"){
						//After check the path,then add the osd model into the table
						AddOSDItemInTable();
					}
