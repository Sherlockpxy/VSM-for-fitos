...
def add_new_osd(request):
    template = "vsm/devices-management/add_osd_fh.html"
    context = {}
    #get the server list
...
def add_new_osd_action_fh(request):
    data = json.loads(request.body)
    print 'data----8888==',data
    #vsmapi.add_new_disks_to_cluster(request,data)
    vsmapi.add_batch_new_physical_disks_to_cluster(request,data)
    status_json = {"status":"OK"}
    status_data = json.dumps(status_json)
    return HttpResponse(status_data)
...
def check_physical_device_path(request):
    search_opts = json.loads(request.body)
    server_id = search_opts["server_id"]
    physical_device_path = search_opts["physical_device_path"]
    ret = vsmapi.get_available_physical_disks(request, search_opts={
                "server_id": server_id
                ,"path":[physical_device_path]
            })
...
def get_available_physical_disks(request):
    search_opts = json.loads(request.body)
    server_id = search_opts["server_id"]
