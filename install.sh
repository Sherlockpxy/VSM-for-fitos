...
[vsmrepo]
name=vsmrepo
baseurl=http://controller_path/opt/vsmrepo
gpgcheck=0
enabled=1
proxy=_none_
EOF
...
[vsmdeprepo]
name=vsmdeprepo
baseurl=http://controller_path/opt/vsm-dep-repo
gpgcheck=0
enabled=1
proxy=_none_
EOF

    $SUDO sed -i s/controller_path/$CONTROLLER_ADDRESS/g vsm.repo
    $SUDO sed -i s/controller_path/$CONTROLLER_ADDRESS/g vsm-dep.repo 
...
#    $SSH $USER@$1 "$SUDO mv /tmp/vsm-dep-repo /opt"
    ...
    $SCP pip.conf $USER@$1:/root/.pip
    $SCP requirements.txt $USER@$1:/root    
    $SCP /root/hosts $USER@$1:/root
}
...
#        exit 1
#    else
#        if [[ $OS_KEYSTONE_HOST ]] && [[ $OS_KEYSTONE_ADMIN_TOKEa ]]; then
...
#        set_iptables_and_selinux $CONTROLLER_ADDRESS
        ...
        $SSH $USER@$CONTROLLER_ADDRESS "$SUDO yum install -y wrapt vsm vsm-deploy vsm-dashboard python-vsmclient"
        $SSH $USER@$CONTROLLER_ADDRESS "$SUDO env PATH=$PATH:$PATH_N preinstall controller"
        setup_remote_controller
        $SSH $USER@CONTROLLER_ADDRESS "$SUDO mkdir -p /tmp/vsm-dep-repo; cd /opt;\
        for i in `ls`; do if [[ -d $i ]]; then $SUDO cp $i/*.rpm /tmp/vsm-dep-repo >/dev/null 2>&1; fi; done"
...
#        $SUDO sed -i "s/keepcache=0/keepcache=1/g" /etc/yum.conf
        ...
        if [ `env PATH=$PATH:$PATH_N cluster_manifest|grep error|wc -l` -gt 0 ]; then
            ...
            if [[ $OS_KEYSTONE_HOST ]] && [[ $OS_KEYSTONE_ADMIN_TOKEN ]]; then
                ...
                if [[ `$SUDO service openstack-keystone status|grep running|wc -l` == 1 ]]; then $SUDO systemctl stop openstack-keystone; fi
            else
                $SUDO env PATH=$PATH:$PATH_N vsm-controller
            fi
        fi
#        $SUDO mkdir -p /tmp/vsm-dep-repo; cd /opt;\
#        for i in `ls | grep vsm`; do
#            if [[ -d $i ]] && [[ `ls $i|wc -l` -gt 0 ]]; then
#                $SUDO cp $i/*.rpm /tmp/vsm-dep-repo >/dev/null 2>&1
#            fi
#        done
#        cd /tmp/vsm-dep-repo
...
#    $SUDO sed -i "/^\[auth_key\]$/,/^\[.*\]/ s/^.*-.*$/$TOKEN/" $MANIFEST_PATH/$1/server.manifest
    OLD_TOKEN=$(sed -n '/^[0-9a-z]\{8,\}/p' $MANIFEST_PATH/$1/server.manifest)
    $SUDO sed -i s/$OLD_TOKEN/$TOKEN/g $MANIFEST_PATH/$1/server.manifest
...
function install_agent() {
    ...
    check_manifest $1
    set_remote_repo $1
    $SSH $USER@$1 "$SUDO pip install -r requirements.txt"
#    set_iptables_and_selinux $1
    $SSH $USER@$1 "$SUDO yum install -y vsm vsm-deploy"
    # $SSH $USER@$1 "$SUDO yum install -y vsm"
    ...
    $SSH $USER@$1 "systemctl restart vsm-agent"
    $SSH $USER@$1 "systemctl restart vsm-physical"
}
...
# OS_KEYSTONE_ADMIN_TOKEa, you have shared the keystone with openstack cluster.
...
$SUDO systemctl restart vsm-conductor
$SUDO systemctl restart vsm-scheduler
